...
import UserManualDocPrint from "./atp-doc-print.component";
import { Formik, Form, Field, ErrorMessage } from "formik";
import * as Yup from "yup";
import { getEmployeeMasterList } from "services/admin.serive";
import AlertConfirmation from "common/AlertConfirmation.component";
import { useNavigate } from "react-router-dom";
import { SRSRecommendReview, SRSReForward, SRSReturnReview } from "services/srs.service";
import { ATPRecommendReview, ATPReForward, ATPReturnReview } from "services/atp.service";

const validationSchema = Yup.object({
  remarks: Yup.string().required("Remarks are required"),
});

const ATPDocxPreview = () => {
  ...
  const handleAction = async(actionType, values) => {
    ...
    if(!confirm) return ;
       ...
       let response;
    if(actionType==="RETURN"){
      response = await ATPReturnReview(remarks,row?.docVersionReleaseId);

    }

     if(actionType==="RECOMMEND"){
      response = await ATPRecommendReview(remarks,row?.docVersionReleaseId);

    }

     if(actionType==="FORWARD"){
      response = await ATPReForward(remarks,row?.docVersionReleaseId);
     ...
     if (response) {
      Swal.fire({
        icon: "success",
        title: "Success",
        text: "ATP Document" + msgStatus+ "successfully",
    ...
    <>
      ...
      <div className="card m-4 shadow-lg border-0 rounded-4">
        ...
        >
            ...
            {statusCode === "RETURN" ? "ATP Docx  Forward" : "ATP Docx Preview"}
        </div>

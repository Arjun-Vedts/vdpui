...
import { userManualApproved, userManualReturnApproval } from "services/usermanual.service";
...
const UserManualApproved = () => {
  ...
  const handleAction = async(actionType, values) => {
    ...
    if(!confirm) return ;
       ...
       let response;
    if(actionType==="RETURN"){
      response = await userManualReturnApproval(remarks,row?.docVersionReleaseId);

    }

     if(actionType==="APPROVAL"){
      response = await userManualApproved(remarks,row?.docVersionReleaseId);
...
export default UserManualApproved;

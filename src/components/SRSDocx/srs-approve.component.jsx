...
const SRSApproved = () => {
  ...
  const handleAction = async(actionType, values) => {
    ...
    if(!confirm) return ;
       ...
       let response;
    if(actionType==="RETURN"){
      response = await SRSReturnApproval(remarks,row?.docVersionReleaseId);

    }

     if(actionType==="APPROVAL"){
      response = await SRSApproval(remarks,row?.docVersionReleaseId);
     ...
     if (response) {
      Swal.fire({
        icon: "success",
        title: "Success",
        text: "SRS DOCX" + msgStatus+ "successfully",

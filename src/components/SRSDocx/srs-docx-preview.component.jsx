...
import styles from "./srsDocxPreview.module.css";
import UserManualDocPrint from "./srs-doc-print";
...
const SRSDocxPreview = () => {
  ...
  const handleAction = async(actionType, values) => {
    ...
    if(!confirm) return ;
       ...
       let response;
    if(actionType==="RETURN"){
      response = await SRSReturnReview(remarks,row?.docVersionReleaseId);

    }

     if(actionType==="RECOMMEND"){
      response = await SRSRecommendReview(remarks,row?.docVersionReleaseId);

    }

     if(actionType==="FORWARD"){
      response = await SRSReForward(remarks,row?.docVersionReleaseId);
    ...
    <>
      ...
      <div className="card m-4 shadow-lg border-0 rounded-4">
        ...
        >
            ...
            {statusCode === "RETURN" ? "SRS Docx  Forward" : "SRS Docx Preview"}
        </div>

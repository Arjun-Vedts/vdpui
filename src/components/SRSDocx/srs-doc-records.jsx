...
import SRSDocHtmlComponent from "./srs-doc-html";
import { getSRSDocTransactionList, getSRSDocVersionDtoListByProject } from "services/srs.service";
import SRSDocAddIssueDialog from "components/SRSDocx/srs-add-isssue-dialog";
import SRSDocPrint from "./srs-doc-print";
import SRSForwardComponent from "./srs-forward-version-dialog.component";



const SRSDocRecordsComponent = ({props}) => {

  
      const wordConversion = (data) => {
        const componentId = 'srs-doc-html';
      
        let ComponentToRender = SRSDocHtmlComponent;
...
const handleAddIssueConfirm = async (response)=>{
  if(response.docVersionReleaseId > 0) {
          const docTrans  = await getSRSDocTransactionList();
...
const fetchUserManualDocVersionData = async () => {
  ...
  try {
    let projectIdData = initialValueOfProjectSel.projectId;
      const projectSelDto = {
        ...
        const srsdocVersionListByProject = await getSRSDocVersionDtoListByProject(projectSelDto);
 ...
 const openModal = (proejctiD, row) => {
  ...
  const openReviewModal = (proejctiD, row) => {
    ...
    window.open('/SRSDocxPreview', '_self');
  };

  const openApprovedModal = (proejctiD, row) => {
    localStorage.setItem('UMPreviewData', JSON.stringify({ row, mode: 'approvalPending' }));
    window.open('/SRSApproved', '_self');
  };
...
const fetchData = async () => {
 ...
 const openTran =  (item) => {
       ...
       // console.log('userManualDocTrans--',userManualDocTrans)
       // console.log('release id--',data.docVersionReleaseId)
        localStorage.setItem('transaction', JSON.stringify(trans));
        window.open('/srs-docs-trans', '_blank');
    }
...
const getMappedData = () => {
  //  console.log(SRSDocVersionListByProject)
...
switch (status) {
  case 'SRSDocContent':
    return <SrsAddContentEditorComponent versionElements={versionElementData} ></SrsAddContentEditorComponent>;
      ...
      <Box className="card-body">
        ...
        <Grid container alignItems="center" justifyContent={"flex-start"} spacing={2}>
          ...
          <Grid item xs={9} sx={{ textAlign: "left" }}>
              <h3 style={{ margin: 0, color:'#484848'}}>SRS Record</h3>
          </Grid>
...
export default withRouter(SRSDocRecordsComponent);